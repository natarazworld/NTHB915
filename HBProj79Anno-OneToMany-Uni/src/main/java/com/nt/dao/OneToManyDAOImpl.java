...
public class OneToManyDAOImpl implements OneToManyDAO {
	...
	public void saveData() {
		...
		try(Session ses=HibernateUtil.getSession()){
			  //prepare objects
			Set<BankAccount> accounts=Set.of(new BankAccount("savings","HDFC",646565.0),
					                                                       new BankAccount("savings","AXIS",616665.0));
			 Person per=new Person("naveen","hydi",accounts); 
				//begin Tx
	...
	public void deleteDataUsingParent() {
		...
		try(Session ses=HibernateUtil.getSession()){
			//Load parent object
			Person per=ses.get(Person.class, 141);
			//begin the Tx
			tx=ses.beginTransaction();
			//delete obj
				ses.delete(per);
	...
	public void deleteOnlyChildsOfAParent() {
		...
		try(Session ses=HibernateUtil.getSession()){
			//Load parent object
			Person per=ses.get(Person.class, 142);
	...
	public void loadDataUsingParentAndQBC() {
		...
		try(Session ses=HibernateUtil.getSession()){
			  //HB QBC logic
			Criteria  ct=ses.createCriteria(Person.class);
			List<Person> list=ct.list();
			list.forEach(per->{
				System.out.println("parent::"+per);
				Set<BankAccount> childs=per.getAccounts();
				childs.forEach(acc->{
					System.out.println("Child::"+acc);
				});
			});
